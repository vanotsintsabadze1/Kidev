...
| Formatting | Define C# formatting in the repository-root `.editorconfig` and enforce formatting and analyzer violations in CI. Exclude EF-generated files under `Kidev.Storage.PostgreSQL/Migrations` and `Kidev.Dashboard/Identity/Migrations` from formatting and analyzers. |
...
| Dashboard architecture | Ship the dashboard as a Razor Class Library plugin, never a standalone executable. Expose `AddKidevDashboard` and `MapKidevDashboard` with an isolated `Kidev` MVC area and packaged static assets. The consuming host owns startup, storage, optional Identity configuration, middleware, migrations, bootstrap, and workers; dashboard registration never starts workers or synchronizes jobs. Keep optional local Identity entities, context, and EF-generated migrations in `Kidev.Dashboard/Identity`, using a separate schema/history table. Local NuGet-consumer demos belong only in ignored `Kidev.TestApp/`, outside the solution. |
| Dashboard access | Use local ASP.NET Core Identity accounts with Administrator-only dashboard access and account creation. Bootstrap the first administrator explicitly through CLI setup; no anonymous signup or automatic setup at normal startup. |
| Dashboard design | Maintain finalized decisions in `DESIGN.md`. Use compact industrial MVC layouts with shared Razor partials, CSS tokens, dark/light themes, and real persisted data. Name the live page Visualization and use `/Visualization` and `/Visualization/Snapshot` beneath the dashboard prefix, without Factory route aliases; retain the factory visual metaphor and internal naming. Do not fabricate unsupported metrics or controls. |
